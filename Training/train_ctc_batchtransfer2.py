#Fine tuning using mix tunning dataset
import tensorflow as tf
import numpy as np
import model.ctcmodel7_transfer_1RNN as mymodel
from util import *
import re
...
#MAX_test_all.append(280)
#MAX_test_all.append(280)
inputpart=[3,3,6,6,6,6,6]#W C T I M R P
FEATURE=np.array([105])
encode=40
OUTPUT=15
CELL_all=[]
...
Training_in=np.load('./data/'+datafolder+'/transfer_value_beforeRNN_rnnin7800.npy')
...
Training_in=Training_in[:,ind[0:75],:]
Training_label=Training_label[ind[0:75],:]
Training_length=Training_length[ind[0:75]]
Training_labelnum=Training_labelnum[ind[0:75]]


Training_inR=np.load('./data/'+realfolder+'/validation_set/transfer_value_beforeRNN_rnnin7800_sep.npy')
...
Testing_in=np.load('./data/'+datafolder+'/testtransfer_value_beforeRNN_rnnin7800.npy')
...
Testing_inR=np.load('./data/'+realfolder+'/testing_set/transfer_value_beforeRNN_rnnin7800.npy')
...
for CELL in CELL_all:
  for layer in Layer_all:
    savedir='Batchtranfer/modelRNNlessDHG4'
    ...
    with tf.name_scope('Testing_modelR'):
        testmodelR=mymodel.model(FEATURE, OUTPUT, CELL,layer,True)
    ...
    with tf.Session() as sess:
       ...
       # D=sess.run(var)
		...
		#uncomment to keep using the learned paramter to train
